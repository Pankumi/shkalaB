...
async function getSwings() {
  try {
    ...
    // перебор свчок з локальної бд для заміни кожної на менші інтервали
...
console.log(`${hours}:${minutes}:${seconds}`);
      ...
      const transformedData = btcusdtData.map((item) => {
        ...
        const low = item[3];
        const high = item[2];
        const priceChange = (parseFloat(high) - parseFloat(low)) / parseFloat(low) * 100;
      ...
      if (transformedData.length !== 0) {
        await BtcusdtMinute.insertMany(transformedData)
          ...
          .then(async () => {
            ...
            console.log("delete >>>", id, startDate,`${hours}:${minutes}:${seconds}`);
          })
